...
public class WMMGameOptionsHarmony
{
    ...
    private static int InitialGamePrefValue = 500;
    ...
    public class WMMCustomGameOptionsHarmony_Init : IModApi
    {
    ...
    public class PatchXUiC_GamePrefSelectorInit
    {
        static bool Prefix(XUiC_GamePrefSelector __instance)
        {
            if (__instance.ViewComponent.ID.StartsWith(CustomNamePrefix))
            {
                ...
                __instance.CustomAttributes.SetAttribute(CustomAttrGamePrefSelectorId, __instance.ViewComponent.ID);
            ...
            ref GamePrefs.EnumType ___valueType)
        {
            ...
            var id = __instance.CustomAttributes.GetStringAttribute(CustomAttrGamePrefSelectorId);
            if (id.StartsWith(CustomNamePrefix))
            {
                ...
                if (!WMMGameOptions.HasValue(id))
                {
                    switch (___valueType)
                    {
                        case GamePrefs.EnumType.Int:
                            WMMGameOptions.SetValue(id, __instance.CustomAttributes.GetIntAttribute(CustomAttrDefaultValue));
                            break;
                        case GamePrefs.EnumType.String:
                            WMMGameOptions.SetValue(id, __instance.CustomAttributes.GetStringAttribute(CustomAttrDefaultValue));
                            break;
                        case GamePrefs.EnumType.Bool:
                            WMMGameOptions.SetValue(id, __instance.CustomAttributes.GetBoolAttribute(CustomAttrDefaultValue));
                            break;
                    }
                }
            }
    ...
    public class PatchXUiC_GamePrefSelectorControlCombo_OnValueChanged
    {
            ...
            ref GamePrefs.EnumType ___valueType)
        {
            if (IsCustomGameOption(__instance))
            {
                ...
                {
                    case GamePrefs.EnumType.Int:
                        WMMGameOptions.SetValue(id, _newValue.IntValue);
                        break;
                    case GamePrefs.EnumType.String:
                        WMMGameOptions.SetValue(id, _newValue.StringValue);
                        break;
                    case GamePrefs.EnumType.Bool:
                        WMMGameOptions.SetValue(id, _newValue.IntValue == 1);
                        break;
                }
                    ...
                    case GamePrefs.EnumType.Int:
                        {
                            ...
                            {
                                WMMGameOptions.SetValue(id, value);
                            }
                            break;
                        }
                    case GamePrefs.EnumType.String:
                        WMMGameOptions.SetValue(id, _text);
                        break;
                }
            ...
            ref XUiC_TextInput ___controlText)
        {
            if (IsCustomGameOption(__instance))
            {
                ...
                switch (___valueType)
                {
                    case GamePrefs.EnumType.Int:
                        {
                            if (___isTextInput)
                            {
                                ___controlText.Text = WMMGameOptions.GetInt(id).ToString();
                                break;
                            }

                            for (int i = 1; i < ___controlCombo.Elements.Count; i++)
                            {
                                if (___controlCombo.Elements[i].IntValue == WMMGameOptions.GetInt(id))
                                {
                    ...
                    case GamePrefs.EnumType.String:
                        {
                            if (___isTextInput)
                            {
                                ___controlText.Text = WMMGameOptions.GetString(id);
                                break;
                            }

                            for (int i = 1; i < ___controlCombo.Elements.Count; i++)
                            {
                                if (___controlCombo.Elements[i].StringValue == WMMGameOptions.GetString(id))
                                {
                                    ___controlCombo.SelectedIndex = i;
                                    break;
                                }
                    ...
                    case GamePrefs.EnumType.Bool:
                        ___controlCombo.SelectedIndex = WMMGameOptions.GetBool(id) ? 1 : 0;
                        break;
                }
                    ...
                    case GamePrefs.EnumType.Int:
                        {
                            ...
                            __result = intValue == __instance.CustomAttributes.GetIntAttribute(CustomAttrDefaultValue);
                            break;
                        }
                    case GamePrefs.EnumType.String:
                        {
                            if (___isTextInput)
                            {
                                __result = ___controlText.Text == __instance.CustomAttributes.GetStringAttribute(CustomAttrDefaultValue);
                                break;
                            }
                            __result = ___controlCombo.Value.StringValue == __instance.CustomAttributes.GetStringAttribute(CustomAttrDefaultValue);
                            break;
                        }
                    case GamePrefs.EnumType.Bool:
                        __result = ___controlCombo.Value.IntValue == 1 == __instance.CustomAttributes.GetBoolAttribute(CustomAttrDefaultValue);
                        break;
                }
    ...
    public class PatchXUiC_NewContinueGameSaveGameOptions
    {
        static void Postfix(XUiC_NewContinueGame __instance)
        {
            ...
            WMMGameOptions.SaveGameOptions();
        }
    }
    ...
    public class PatchXUiC_GamePrefSelectorParseAttribute
    {
            ...
            ref GamePrefs.EnumType ___valueType)
        {
            ...
            {
                __instance.CustomAttributes.SetAttribute(CustomAttrDefaultValue, _value);
    ...
    public class PatchScoreConfigurationCheckFeatureStatus
    {
        ...
        static bool Prefix(ref bool __result, string strClass, string strFeature)
        {
            ...
            if (WMMGameOptions.HasValue(variableName))
            {
                __result = WMMGameOptions.GetBool(variableName);
                return false;
            }
    ...
    [HarmonyPatch(new Type[] { typeof(EnumGamePrefs), typeof(object) })]
    public class Patch_GamePrefs_SetObjectInternal
    {
        static bool Prefix(GamePrefs __instance, EnumGamePrefs _eProperty, object _value)
        {
            // hide error message because enum doesnt match vanilla enums
            int num = (int)_eProperty;

            if (num >= InitialGamePrefValue)
            {
                return false;
            }
